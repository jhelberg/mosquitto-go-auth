...
* Files
* PostgreSQL
* PostgreSQL with MQTT user as PostgreSQL user
...
The `postgresp` is similar to the `postgres` backend, except for
ignoring the user/password options and ignoring the superquery. It
also adds a different acl-query: `pgp_aclquery` which is expected to
return true or false given the topic.

`postgresp` will use the MQTT login credentials to logon onto the
PostgreSQL database, moving authentication down into the RDBMS. The
`pg_userquery` option then can be a simple `select 1`.
The `pg_aclquery` can be anything, but as the RDBMS will implement
authorization, based on the user logged on, boils down to some simple
select like: `select * from allowedtopics($2)`.

The `postgresp` backend and Row Level Security with end-user database
roles are a seamless fit.

Even better, the added option `pgp_aclquery` is expected to return a boolean,
given the topic as `$3`: true or false. This allows the RDBMS to match
topics and figure out what is allowed and what not. For users who may
use thousands of topics, this is far more practical than sending these
over and let the mosquitto code go over them one-by-one before
deciding things are ok or not. The use of a non-empty not-failing
`pgp_aclquery` will ignore any configured `pg_aclquery`.

Connection management is an issue in `postgresp`, especially caching
connections and re-connecting in large environments. It is felt
however that no other system can implement
authentication/authorization in a non-leaky, GDPR compliant way than
leaving it all to a RLS-controlled RDBMS.

Considering the above, superusers cannot exist, hence ignoring this
option.
